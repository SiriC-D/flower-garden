...
/** Better hash so flowers scatter instead of stacking */
function positionForId(id) {
  ...
  // Keep flowers away from edges a bit
  const x = 6 + (Math.abs(h1) % 8800) / 100; // 6–94
  const y = 8 + (Math.abs(h2) % 8200) / 100; // 8–90
  return { x, y };
}
...
export default function FlowerGarden() {
  ...
  // ========== MAIN ==========
  return (
    <div className="h-screen flex flex-col bg-[#F7F4ED] text-[#2E3A2E] overflow-hidden">
      {/* Header */}
      <header className="shrink-0 border-b border-[#E0D9C8] bg-[#FBF9F4] px-4 md:px-6 py-3">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <h1 className="text-xl md:text-2xl font-bold tracking-tight text-[#1B3A1B] truncate">
              Community Flower Garden
            </h1>
            <p className="text-xs md:text-sm text-[#6B7C6B] hidden sm:block">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="p-2 rounded-full border border-[#D4CBB8] hover:bg-white transition lg:hidden"
              title="Toggle plant panel"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-3 py-2 rounded-full bg-white border border-[#D4CBB8] text-sm font-medium hover:bg-[#F0EBE0] transition"
            >
              <ImageIcon size={16} /> <span className="hidden sm:inline">Gallery</span>
            </button>
            <button
              onClick={() => setRightOpen((v) => !v)}
              className="p-2 rounded-full border border-[#D4CBB8] hover:bg-white transition hidden lg:inline-flex"
              title="Toggle stats"
            >
      ...
      <div className="flex-1 flex min-h-0 relative">
        ...
        <aside
          className={`
            shrink-0 border-r border-[#E0D9C8] bg-[#FBF9F4] overflow-y-auto
            transition-all duration-300 ease-in-out
            ${leftOpen ? "w-[280px] opacity-100" : "w-0 opacity-0 overflow-hidden border-r-0"}
            absolute lg:static inset-y-0 left-0 z-20 lg:z-0 shadow-lg lg:shadow-none
          `}
        >
          <div className="p-4 space-y-5 w-[280px]">
            {/* collapse on desktop */}
            <div className="hidden lg:flex justify-end">
              <button onClick={() => setLeftOpen(false)} className="p-1.5 rounded-lg hover:bg-[#EDE8DC] text-[#8A9A8A]" title="Collapse">
            ...
            {/* Name */}
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-wider text-[#8A9A8A] mb-1.5">Gardening as</p>
              {nameEditing ? (
                <form onSubmit={(e) => { e.preventDefault(); saveName(gardenerName); }} className="flex gap-2">
                  ...
                  <button type="submit" className="px-3 py-2 rounded-lg bg-[#66BB6A] text-white text-sm font-semibold">Save</button>
                </form>
              ) : (
                <button onClick={() => setNameEditing(true)} className="flex items-center gap-2 text-base font-semibold text-[#1B3A1B] hover:opacity-70">
                  {gardenerName || "Anonymous"} <Pencil size={13} className="text-[#8A9A8A]" />
                </button>
              )}
            </div>
            ...
            <div className="flex rounded-xl border border-[#D4CBB8] overflow-hidden text-sm font-medium">
              <button onClick={() => setMode("quick")}
                className={`flex-1 py-2 flex items-center justify-center gap-1.5 ${mode === "quick" ? "bg-[#E8F5E9] text-[#2E7D32]" : "bg-white text-[#6B7C6B]"}`}>
                <Flower2 size={14} /> Quick plant
              </button>
              <button onClick={() => setMode("draw")}
                className={`flex-1 py-2 flex items-center justify-center gap-1.5 ${mode === "draw" ? "bg-[#E8F5E9] text-[#2E7D32]" : "bg-white text-[#6B7C6B]"}`}>
                <Pencil size={14} /> Draw
              </button>
            </div>
            ...
            {mode === "quick" ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {QUICK_COLORS.map((c) => (
                      <button key={c} onClick={() => setSelectedColor(c)}
                        className={`w-7 h-7 rounded-full border-2 transition ${selectedColor === c ? "border-[#1B3A1B] scale-110" : "border-white shadow"}`}
                        style={{ backgroundColor: c }} />
                    ))}
                  </div>
                  <div className="mt-2 h-1.5 rounded-full" style={{ backgroundColor: selectedColor }} />
                </div>
                <button onClick={plantQuick} disabled={cooldownRemaining > 0}
                  className="w-full py-3 rounded-xl bg-[#66BB6A] hover:bg-[#43A047] text-white font-bold shadow-sm transition disabled:opacity-50 flex items-center justify-center gap-2">
                  <Leaf size={18} />
              ...
              <>
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-[#8A9A8A] mb-2">Colors</p>
                  <div className="flex flex-wrap gap-2">
                    {QUICK_COLORS.map((c) => (
                      <button key={c} onClick={() => setSelectedColor(c)}
                        className={`w-7 h-7 rounded-full border-2 ${selectedColor === c ? "border-[#1B3A1B] scale-110" : "border-white shadow"}`}
                        style={{ backgroundColor: c }} />
                    ))}
                ...
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-[#8A9A8A] mb-2">Brush</p>
                ...
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-[#8A9A8A] mb-2">Thickness</p>
                  <div className="flex gap-2">
                    {THICKNESS_OPTIONS.map((s) => (
                      <button key={s} onClick={() => setThickness(s)}
                        className={`w-9 h-9 rounded-full flex items-center justify-center border-2 ${
                          thickness === s ? "border-[#1B3A1B] bg-[#E8F5E9]" : "border-white bg-white shadow"
                        }`}>
                        <div className="rounded-full bg-[#2E7D32]" style={{ width: s * 1.5, height: s * 1.5 }} />
                      </button>
                    ))}
                  </div>
                </div>
                <div className="flex gap-2">
                  <button onClick={undo} disabled={!canUndo} className="flex-1 py-1.5 rounded-lg border text-xs disabled:opacity-40">
                    <Undo2 size={12} className="inline mr-1" /> Undo
                  </button>
                  <button onClick={redo} disabled={!canRedo} className="flex-1 py-1.5 rounded-lg border text-xs disabled:opacity-40">
                ...
                <div className="flex gap-2">
                  <button onClick={clearCanvas} className="flex-1 py-2 rounded-xl border border-[#D4CBB8] text-xs font-medium">
                    <RotateCcw size={12} className="inline mr-1" /> Clear
                  </button>
                  <button onClick={plantDrawn} disabled={cooldownRemaining > 0}
                    className="flex-[2] py-2 rounded-xl bg-[#66BB6A] text-white font-bold text-xs disabled:opacity-50">
        ...
        {/* CENTER GARDEN — takes all remaining space */}
        <main className="flex-1 relative min-w-0 bg-[#E8F5E0] overflow-hidden">
          {/* soft petals */}
          <div className="absolute inset-0 pointer-events-none opacity-30">
            {Array.from({ length: 24 }).map((_, i) => (
              <div key={i} className="absolute rounded-full bg-pink-200"
                style={{
                  width: 10 + (i % 6) * 5,
                  height: 7 + (i % 4) * 4,
                  left: `${(i * 19) % 100}%`,
          ...
          {loading ? (
            ...
            flowers.map((f) => {
              const pos = positionForId(f.id);
              return (
                <div
                  key={f.id}
                  className="absolute transition-transform hover:scale-125 hover:z-10 cursor-default"
                  style={{
                    left: `${pos.x}%`,
                    top: `${pos.y}%`,
                ...
                >
                  <img src={f.image} alt="" className="w-full h-full object-contain drop-shadow-md" />
                </div>
              );
            })
          )}
        ...
        <aside
          className={`
            shrink-0 border-l border-[#E0D9C8] bg-[#FBF9F4] overflow-y-auto
            transition-all duration-300 ease-in-out
            ${rightOpen ? "w-[240px] opacity-100" : "w-0 opacity-0 overflow-hidden border-l-0"}
            hidden lg:block
          `}
        >
          <div className="p-4 space-y-5 w-[240px]">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-base font-bold text-[#1B3A1B]">Garden Stats</h3>
                <p className="text-[10px] text-[#8A9A8A]">Live community activity</p>
              </div>
              <button onClick={() => setRightOpen(false)} className="p-1.5 rounded-lg hover:bg-[#EDE8DC] text-[#8A9A8A]" title="Collapse">
                <PanelRightClose size={16} />
              </button>
            </div>

            <div className="rounded-2xl bg-[#E8F5E9] p-3.5 flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-white flex items-center justify-center text-[#66BB6A]">
                <Flower2 size={18} />
              </div>
              <div>
                <p className="text-xl font-bold text-[#1B3A1B]">{flowers.length}</p>
                <p className="text-[10px] text-[#558B2F]">Flowers planted</p>
              </div>
            </div>

            <div className="rounded-2xl bg-[#FFF3E0] p-3.5 flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-white flex items-center justify-center text-[#FB8C00]">
                <Users size={18} />
              </div>
              <div>
                <p className="text-xl font-bold text-[#1B3A1B]">{onlineCount}</p>
                <p className="text-[10px] text-[#E65100]">Gardeners online</p>
              </div>
            </div>

            <div>
              <p className="text-[10px] font-semibold uppercase tracking-wider text-[#8A9A8A] mb-2">Species Distribution</p>
            ...
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-wider text-[#8A9A8A] mb-2">Recent Activity</p>
              <div className="space-y-2.5">
                {recent.map((f) => {
                  const type = FLOWER_TYPES.find((t) => t.id === f.flower_type) || { emoji: "🌸", label: "Custom" };
                  return (
                    <div key={f.id} className="flex items-start gap-2 text-sm">
                      <span className="text-base leading-none mt-0.5">{type.emoji}</span>
                      <div className="min-w-0">
                        <p className="font-medium text-[#1B3A1B] truncate text-[13px]">{f.planter_name || "Anonymous"}</p>
