    ...
    const blooms = [
      ...
      .map(([cx, cy], i) => {
        const petals = Array.from({ length: 5 }, (_, j) => {
          ...
          return `<circle cx="${cx + Math.cos(a) * 10}" cy="${cy + Math.sin(a) * 10}" r="7" fill="${color}" opacity="${0.85 + (i % 2) * 0.1}"/>`;
        }).join("");
  ...
  const layers = [
    ...
    .map(({ r, n, o }) =>
      Array.from({ length: n }, (_, i) => {
        ...
        return `<circle cx="${60 + Math.cos(a) * r * 0.55}" cy="${48 + Math.sin(a) * r * 0.55}" r="${r * 0.45}" fill="${color}" opacity="${o}"/>`;
      }).join("")
    )
    .join("");
...
/** Fallback for old flowers without pos_x/pos_y */
function fallbackPos(id) {
  const s = String(id);
  let h1 = 2166136261;
  let h2 = 2166136261;
  ...
  const pad = 60;
  return {
    x: pad + (Math.abs(h1) % (GARDEN_W - pad * 2)),
    y: pad + (Math.abs(h2) % (GARDEN_H - pad * 2)),
    size: 60 + (Math.abs(h1 + h2) % 30),
  };
}

function getFlowerPos(f) {
  if (f.pos_x != null && f.pos_y != null) {
    return {
      x: Number(f.pos_x),
      y: Number(f.pos_y),
      size: 60 + (Math.abs(Number(f.id) || 0) % 30),
    };
  }
  return fallbackPos(f.id);
}

function isSpotCrowded(x, y, list) {
  let n = 0;
  for (const f of list) {
    const p = getFlowerPos(f);
    if (Math.hypot(p.x - x, p.y - y) < MIN_DISTANCE) n++;
  }
  return n >= MAX_NEARBY;
}
...
export default function FlowerGarden() {
  ...
  useEffect(() => {
    ...
    el.scrollLeft = Math.max(0, (GARDEN_W - el.clientWidth) / 2);
    el.scrollTop = Math.max(0, (GARDEN_H - el.clientHeight) / 2);
  }, [loading]);
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "flowers" }, (payload) => {
        setFlowers((prev) =>
          prev.map((f) =>
            f.id === payload.new.id ? { ...f, waters: payload.new.waters ?? f.waters } : f
          )
        );
      })
  ...
  // --- plant at exact click (FIXED coords) ---
  const plantAt = async (x, y) => {
    ...
    if (isSpotCrowded(x, y, flowers)) {
      setMessage("Garden is full here — try elsewhere 🌿");
    ...
    try {
      ...
      setMessage(`${selectedType.emoji} planted!`);
      setTimeout(() => setMessage(""), 2000);
    } catch (err) {
      console.error(err);
  ...
  /** FIXED: use only getBoundingClientRect — do NOT add scrollLeft/scrollTop */
  const handleGardenClick = (e) => {
    if (e.target.tagName === "IMG") return;
    const rect = e.currentTarget.getBoundingClientRect();
    const x = e.clientX - rect.left;
    const y = e.clientY - rect.top;
    const cx = Math.max(40, Math.min(GARDEN_W - 40, x));
    const cy = Math.max(40, Math.min(GARDEN_H - 40, y));
    plantAt(cx, cy);
  };
  ...
  const plantDrawn = async () => {
    ...
    const x = GARDEN_W / 2 + (Math.random() - 0.5) * 180;
    const y = GARDEN_H / 2 + (Math.random() - 0.5) * 180;
    try {
      const { data, error } = await supabase
        .from("flowers")
        .insert([{
    ...
    <div className="h-screen flex flex-col bg-[#F5F0E8] text-[#2E3A2E] overflow-hidden">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-[#8A9A8A] hidden sm:block leading-tight">
                Click the garden to plant · grows live
              </p>
            </div>
          </div>
      ...
      <div className="flex-1 flex min-h-0 relative">
        <aside className={`
          ...
          ${leftOpen ? "w-[260px] opacity-100" : "w-0 opacity-0 overflow-hidden border-r-0"}
          ...
          <div className="p-4 space-y-4 w-[260px]">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {FLOWER_TYPES.map((t) => (
                  <button key={t.id}
                    onClick={() => { setSelectedType(t); setSelectedColor(t.color); }}
            ...
            <div className="text-center text-[12px] text-[#558B2F] bg-[#E8F5E9] rounded-xl py-3 px-2 font-medium">
              {cooldownRemaining > 0
        ...
        <main ref={gardenScrollRef} className="flex-1 min-w-0 overflow-auto bg-[#E4EED8]" style={{ scrollbarWidth: "thin" }}>
          ...
          >
            <div className="absolute inset-0 pointer-events-none opacity-30"
              style={{
                backgroundImage: `
                  radial-gradient(ellipse at 25% 30%, rgba(255,200,220,0.3) 0%, transparent 50%),
                  radial-gradient(ellipse at 70% 60%, rgba(255,220,180,0.25) 0%, transparent 45%)
                `,
              }}
            />
            {loading ? (
              <div className="absolute inset-0 flex items-center justify-center text-[#558B2F] text-lg">Loading… 🌱</div>
            ) : flowers.length === 0 ? (
              <div className="absolute inset-0 flex items-center justify-center text-center text-[#558B2F] pointer-events-none">
                <div>
                  <p className="text-4xl mb-2">🌱</p>
                  <p className="text-xl font-semibold">Click to plant</p>
                </div>
              </div>
            ) : (
              flowers.map((f) => {
                const { x, y, size } = getFlowerPos(f);
                return (
                  <div
                    key={f.id}
                    className="absolute hover:scale-110 hover:z-20 transition-transform duration-150 pointer-events-none"
                    style={{
                      left: x,
                      top: y,
                      width: size,
        ...
        <aside className={`
          ...
          ${rightOpen ? "w-[210px] opacity-100" : "w-0 opacity-0 overflow-hidden border-l-0"}
          hidden lg:block
        `}>
          <div className="p-4 space-y-4 w-[210px]">
